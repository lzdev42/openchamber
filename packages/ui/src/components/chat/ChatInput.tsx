...
const ChatInputComponent: React.FC<ChatInputProps> = ({ onOpenSettings, scrollToBottom }) => {
    ...
    const handleSubmit = async (options?: SubmitOptions) => {
        ...
        if (inputMode === 'normal' && normalizedCommand.startsWith('/')) {
            ...
            if (commandName === 'undo' && currentSessionId) {
                ...
                try {
                    await sessionActions.waitForConnectionOrThrow();
                    const userDescription = normalizedCommand.replace(/^\/plan-feature\b/i, '').trim();
                    const userDescriptionLine = userDescription ? `\n\n${userDescription}` : '';
                    const userDescriptionBlock = userDescription
                        ? `\n\nThe user described the feature as:\n${userDescription}\n\nUse this as the starting point for planning. Do not ask the user to repeat what they already wrote above.`
                        : '';
                    const visibleText = await renderMagicPrompt('session.plan.visible', { user_description_line: userDescriptionLine });
                    const instructionsText = await renderMagicPrompt('session.plan.instructions');
                    const fullInstructions = userDescriptionBlock
                        ? `${instructionsText}${userDescriptionBlock}`
                        : instructionsText;
                    await sendMessage(
                        visibleText,
                        providerIdToSend,
                        ...
                        [],
                        agentMentionName,
                        [{ text: fullInstructions, synthetic: true }],
                        variantToSend,
                        inputMode,
                        sendMessageOptions,
                    );
                ...
                try {
                    await sessionActions.waitForConnectionOrThrow();
                    const userDescription = normalizedCommand.replace(/^\/debug\b/i, '').trim();
                    const userDescriptionLine = userDescription ? `\n\n${userDescription}` : '';
                    const userDescriptionBlock = userDescription
                        ? `\n\nThe user described the issue as:\n${userDescription}\n\nUse this as the starting point for the investigation. Do not ask the user to repeat what they already wrote above.`
                        : '';
                    const visibleText = await renderMagicPrompt('session.debug.visible', { user_description_line: userDescriptionLine });
                ...
                try {
                    await sessionActions.waitForConnectionOrThrow();
                    const userDescription = normalizedCommand.replace(/^\/weigh\b/i, '').trim();
                    const userDescriptionLine = userDescription ? `\n\n${userDescription}` : '';
                    const userDescriptionBlock = userDescription
                        ? `\n\nThe user wants to weigh approaches for:\n${userDescription}\n\nUse this as the starting point. Do not ask the user to repeat what they already wrote above.`
                        : '';
                    const visibleText = await renderMagicPrompt('session.weigh.visible', { user_description_line: userDescriptionLine });
                ...
                try {
                    await sessionActions.waitForConnectionOrThrow();
                    const focusArea = normalizedCommand.replace(/^\/explore\b/i, '').trim();
                    const focusAreaLine = focusArea ? ` Focus on: ${focusArea}.` : '';
                    const focusAreaBlock = focusArea
                        ? `\n\nThe user specifically wants to explore: ${focusArea}. Focus the tour on that area while still providing enough context to understand how it fits into the whole.`
                        : '';
                    const visibleText = await renderMagicPrompt('session.explore.visible', { focus_area_line: focusAreaLine });
                    const instructionsText = await renderMagicPrompt('session.explore.instructions');
                    const fullInstructions = focusAreaBlock
                        ? `${instructionsText}${focusAreaBlock}`
                        : instructionsText;
                    await sendMessage(
                        visibleText,
                        providerIdToSend,
    ...
    const updateAutocompleteOverlayPosition = React.useCallback(() => {
        ...
        const desiredWidth = showFileMention ? 520 : showCommandAutocomplete ? containerRect.width - popupMargin * 2 : showSnippetAutocomplete ? 450 : 360;
        const clampedLeft = showCommandAutocomplete
            ? popupMargin
            : Math.max(
                popupMargin,
                        ...
                        <CommandAutocomplete
                            ...
                            style={isDesktopExpanded && autocompleteOverlayPosition
                                ? {
                                    ...
                                    bottom: 'auto',
                                    width: `calc(100% - ${autocompleteOverlayPosition.left * 2}px)`,
