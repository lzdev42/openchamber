...
import { createAirouterRouter } from './lib/airouter/index.js';
...
function shouldSkipCompression(req, res) {
  if (process.env.OCELOT_RUNTIME === 'desktop') {
    return true;
  }
...
const shouldSkipApiCompression = () => {
  if (isEnvFlagEnabled(process.env.OCELOT_SKIP_API_COMPRESSION)) return true;
  if (isEnvFlagEnabled(process.env.OCELOT_COMPRESS_API)) return false;
  if (isEnvFlagDisabled(process.env.OCELOT_COMPRESS_API)) return true;
  return process.env.OCELOT_RUNTIME === 'desktop';
};

const OPENCHAMBER_VERBOSE_REQUEST_LOGS = isEnvFlagEnabled(process.env.OCELOT_VERBOSE_REQUEST_LOGS);
...
const OPENCHAMBER_USER_CONFIG_ROOT = path.join(os.homedir(), '.config', 'ocelot');
...
const OPENCHAMBER_DATA_DIR = process.env.OCELOT_DATA_DIR
  ? path.resolve(process.env.OCELOT_DATA_DIR)
  : path.join(os.homedir(), '.config', 'ocelot');
...
const ENV_SKIP_OPENCODE_START = process.env.OPENCODE_SKIP_START === 'true' ||
                                    process.env.OCELOT_SKIP_OPENCODE_START === 'true';
const ENV_DESKTOP_NOTIFY = (() => {
  if (process.env.OCELOT_DESKTOP_NOTIFY === 'true') {
    return true;
  }
...
const ENV_CONFIGURED_API_PREFIX = normalizeApiPrefix(
  process.env.OPENCODE_API_PREFIX || process.env.OCELOT_API_PREFIX || ''
);
...
const resolveBundledOpenCodeCliPath = (...args) => openCodeEnvRuntime.resolveBundledOpenCodeCliPath(...args);
...
const startManagedOpenCode = async (options) => {
  const result = await openCodeLifecycleRuntime.startManagedOpenCode(options);
  if (result.started) {
    // Mirror the post-start wiring done by bootstrapOpenCodeAtStartup so the
    // on-demand-started managed instance behaves identically to a boot-started one.
...
async function main(options = {}) {
  ...
  const effectiveBindHost = host
    || (typeof process.env.OCELOT_HOST === 'string' && process.env.OCELOT_HOST.trim().length > 0
      ? process.env.OCELOT_HOST.trim()
      : '127.0.0.1');
  const uiPassword = typeof options.uiPassword === 'string'
    ? options.uiPassword
    : (typeof process.env.OCELOT_UI_PASSWORD === 'string' ? process.env.OCELOT_UI_PASSWORD : null);
  if (
  ...
  const apiOnly = options.apiOnly === true || isEnvFlagEnabled(process.env.OCELOT_API_ONLY);
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    ...
    runtimeName: process.env.OCELOT_RUNTIME || 'web',
    serverStartedAt,
    gracefulShutdown,
    getHealthSnapshot: () => {
      ...
      return {
        ...
        bundledCliAvailable: Boolean(resolveBundledOpenCodeCliPath()),
  ...
  const airouterRouter = createAirouterRouter({
    webPort: port,
    configDir: undefined,
    refreshFn: refreshOpenCodeAfterConfigChange,
  });
  app.use('/api/airouter', airouterRouter);
