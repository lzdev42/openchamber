...
import { mintOutsideFileGrant } from '@ocelot/web/server/lib/fs/routes.js';

const execFileAsync = promisify(execFile);

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const isDev = process.env.OCELOT_ELECTRON_DEV === '1' || !app.isPackaged;

const DEEP_LINK_PROTOCOL = 'openchamber';
const UI_PROTOCOL = 'openchamber-ui';
const PACKAGED_APP_USER_MODEL_ID = 'dev.ocelot.desktop';
const DEV_APP_USER_MODEL_ID = 'dev.ocelot.desktop.dev';
...
// ~/Library/Logs/Ocelot/ (not ~/Library/Logs/@ocelot/electron/).
app.setName('Ocelot');
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'Ocelot Dev'));
}
...
// that output never lands in ~/Library/Logs/Ocelot/main.log and we
...
const readAppMetadata = () => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed?.name === '@ocelot/electron' && typeof parsed.version === 'string') {
        return { name: parsed.name, version: parsed.version };
      }
    } catch {
    }
  }
  return { name: '@ocelot/electron', version: app.getVersion() };
};
...
const CHANGELOG_URL = 'https://raw.githubusercontent.com/lzdev42/ocelot/main/CHANGELOG.md';
const GITHUB_BUG_REPORT_URL = 'https://github.com/lzdev42/ocelot/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/lzdev42/ocelot/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `Ocelot detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
const settingsFilePath = () => {
  if (typeof process.env.OCELOT_DATA_DIR === 'string' && process.env.OCELOT_DATA_DIR.trim()) {
    return path.join(process.env.OCELOT_DATA_DIR.trim(), 'settings.json');
  }
  return path.join(os.homedir(), '.config', 'ocelot', 'settings.json');
};
...
const shouldUsePackagedUi = () => {
  if (process.env.OCELOT_ELECTRON_LOAD_SERVER_UI === '1') return false;
  if (process.env.OCELOT_ELECTRON_USE_BUNDLED_UI === '1') return true;
...
import { pathLooksUserConfigured, mergePathValues } from '@ocelot/web/server/lib/opencode/path-utils.js';
...
const spawnLocalServer = async () => {
  ...
  // The server module reads ENV_DESKTOP_NOTIFY / OCELOT_DIST_DIR /
  // OCELOT_RUNTIME at import time (top-level const), so these must be
  ...
  process.env.OCELOT_HOST = bindHost;
  process.env.OCELOT_DESKTOP_LAN_ACCESS_ACTIVE = effectiveLanAccessEnabled ? 'true' : 'false';
  if (lanAccessBlockedByMissingPassword) {
    process.env.OCELOT_DESKTOP_LAN_ACCESS_BLOCKED_REASON = 'missing-password';
  } else {
    delete process.env.OCELOT_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
  }
  process.env.OCELOT_DIST_DIR = resolveWebDistDir();
  process.env.OCELOT_RUNTIME = 'desktop';
  ...
  process.env.OCELOT_OPENCODE_CWD = resolveManagedOpenCodeCwd({
    env: process.env,
    homedir: () => os.homedir(),
  });
  process.env.OCELOT_DESKTOP_NOTIFY = 'true';
  if (desktopUiPassword) {
    process.env.OCELOT_UI_PASSWORD = desktopUiPassword;
  } else {
    delete process.env.OCELOT_UI_PASSWORD;
  }
  process.env.OCELOT_SKIP_API_COMPRESSION = process.env.OCELOT_SKIP_API_COMPRESSION || 'true';
  process.env.NO_PROXY = process.env.NO_PROXY || 'localhost,127.0.0.1';
  process.env.no_proxy = process.env.no_proxy || 'localhost,127.0.0.1';

  // First-launch onboarding: when the user has not yet picked a source
  // (local/bundled/remote), do NOT auto-start a managed OpenCode instance.
  // The web server still boots, but bootstrapOpenCodeAtStartup skips the
  // start step and waits for the renderer to call POST /api/opencode/start
  // after the user picks a source and clicks Start. Once a choice is
  // persisted (initialHostChoiceCompleted=true), normal boot resumes.
  if (readDesktopHostsConfig().initialHostChoiceCompleted) {
    delete process.env.OPENCHAMBER_AWAIT_USER_CHOICE;
  } else {
    process.env.OPENCHAMBER_AWAIT_USER_CHOICE = '1';
  }

  const { startWebUiServer } = await import('@ocelot/web/server/index.js');
...
const computeBootOutcome = ({ envTargetUrl, probe, config, localAvailable }) => {
  console.log('[computeBootOutcome] input=', { envTargetUrl: envTargetUrl || null, probeStatus: probe?.status || null, defaultHostId: config.defaultHostId || null, localAvailable, hostsCount: config.hosts?.length || 0 });
  if (envTargetUrl) {
    const status = probe?.status === 'unreachable'
      ...
      : probe?.status === 'incompatible'
        ...
        : probe?.status === 'wrong-service'
          ? 'wrong-service'
          : 'ok';
    const outcome = { target: 'remote', status, hostId: ENV_OVERRIDE_HOST_ID, url: envTargetUrl };
    console.log('[computeBootOutcome] outcome (envTargetUrl)=', outcome);
    return outcome;
  }

  const defaultId = config.defaultHostId || '';
  if (!defaultId) {
    const outcome = { target: null, status: 'not-configured' };
    console.log('[computeBootOutcome] outcome (not-configured)=', outcome);
    return outcome;
  }

  if (defaultId === LOCAL_HOST_ID) {
    const outcome = localAvailable
      ? { target: 'local', status: 'ok' }
      : { target: 'local', status: 'unreachable' };
    console.log('[computeBootOutcome] outcome (local)=', outcome);
    return outcome;
  }
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ocelot loading icon">
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: 'Connect to Ocelot server?',
...
const resolveInitialUrl = async () => {
  const hmrApiPort = process.env.OCELOT_HMR_API_PORT || '3901';
  const hmrUiPort = process.env.OCELOT_HMR_UI_PORT || '5173';
  ...
  const envTarget = normalizeHostUrl(process.env.OCELOT_SERVER_URL || '');
  const config = readDesktopHostsConfig();
  if (envTarget) {
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/ocelot/credentials'];
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires Ocelot.app to be installed in /Applications');
          }
        } catch (error) {
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About Ocelot', click: () => dispatchAction('about') },
        {
